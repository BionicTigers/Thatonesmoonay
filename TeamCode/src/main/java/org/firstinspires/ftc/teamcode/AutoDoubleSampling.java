...
//          Start lift down motors(2) go down, need method (note to self), reverse one motor
        ...
        nav.pointTurn(110f); //get off hook by going to the right, we are hooked by the butt of the robot
        sleep(100);
        nav.pointTurn(0f); //turn back to straighten self out, left
        sleep(100);
        //read CV sampling
        nav.pointTurn(ralph); //turn right or left to go to the sample
        nav.goDistance(driveForward1); //go to the middle spot at sampling
        sleep(100);
        nav.goDistance(-10f); //back up, after hitting the sampling in the middle
        sleep(100);
        nav.pointTurn(turnLeft1); //turn right to face wall with butt towards depot
        sleep(100);
        nav.goDistance(-43f); // drive towards wall and get close enough to it
        sleep(100);
        nav.pointTurn(rotate); //turn right to face depot and square up with wall with butt towards depot
        sleep(100);
        nav.goDistance(depotDrive); //drive to depot to drop off teamMarker
        sleep(100);

        nav.pointTurn(sampleTurn);
        sleep(100);
        nav.goDistance(sampleDistance);
        sleep(50);
        nav.goDistance(-sampleDistance);
        sleep(50);
        nav.pointTurn((360f - sampleTurn));
        sleep(50);
        nav.goDistance(60f);
        sleep(50);
        ...
        nav.goDistance(craterDrive); //go the forwards toward the crater and park in front of it
    }
}